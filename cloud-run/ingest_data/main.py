...
def ingest_realtime_data(request):
    """Point d'entrée de la Cloud Function exécuté par Cloud Run."""
    ...
    try:
        ...
        for entity in entities:
            ...
            # On applique zfill(3) pour la cohérence des ID avec le référentiel
            station_id = str(station_id).zfill(3)

            available_bike = entity.get("availableBikeNumber", {}).get(
                "value", 0
            )
            ...
            if date_obs:
                # La date de l'API est en UTC -> conversion en heure de Paris
